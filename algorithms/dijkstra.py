...
class DijkstraSSSP:
    """
    Pure Python implementation of Dijkstra
    """
    def __init__(self, matrix, source):
        ...
        # Convert to Adjacency List (Pure Python List of Lists)
        # This matches the preprocessing step in Duan
        self.adj = matrix

        # Pure Python lists for distance
...
# Wrapper function for benchmark.py
def dijkstra(matrix, start_node, end_node):
    solver = DijkstraSSSP(matrix, start_node)
    solver.solve()
    return solver.dist[end_node]

def load_matrix(file_path):
    """
    Зчитує CSV файл з матрицею суміжності у NumPy масив.
    """
    # REWORK FOR NORMAL MATRIX
    try:
...
if __name__ == "__main__":
    ...
    path1, dist1 = dijkstra(matrix, 0, len(matrix)-1)
    print(f"Шлях: {path1}")
